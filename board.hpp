...
enum class Side : std::uint8_t { BLACK, WHITE };

Side getOpponentSide(Side side);

std::ostream &operator<<(std::ostream &os, Side side);

enum class CellState : std::uint8_t { EMPTY, BLACK, WHITE };
...
public:
  ...
  int eval(Side side) const;
  int max_node(const Side side, int depth, int max_depth);
  int min_node(const Side side, int depth, int max_depth);
